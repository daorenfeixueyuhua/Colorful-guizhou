...
import com.gznu.element.dao.login.LoginDao;
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        for (Session session : list) {
            ...
            if (s.isAuthenticated()) {
                ...
                if (username.equals(mangerPrincipals.getUsername()) && !session.getId().equals(sessionId)
        ...
        try {
            mangerInfo = loginDao.selectUserUserPrincipalById(username);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (mangerInfo == null) {
        ...
        // 获取从数据库查询出来的用户密码
        String password = mangerInfo.getPassWord();

        userPrincipal.setUsername(mangerInfo.getUserName());
